...
#define TYPE_FLOAT 2
#define TYPE_PTR 3
#define TYPE_PTR_INT 4
#define TYPE_PTR_CHAR 5
...
void next() {
    ...
    // Handle preprocessor directives - skip # lines
    while (*src == '#') {
        skip_to_eol();
        skip_ws();
    }
    ...
    if (is_alpha(c)) {
        ...
        // Macro expansion not supported - the Rust compiler preprocesses defines
...
void emit_ld_hl_mem(int addr) { emit(0x2A); emit(addr & 0xFF); emit((addr >> 8) & 0xFF); }
...
void gen_unary() {
    ...
    if (tok == TK_STAR) {
        ...
        // Try to determine pointer type before generating code
        // If next token is an identifier, look up its type
        ptr_type = TYPE_PTR;  // default
        if (tok == TK_IDENT) {
            sym = find_sym(tok_str);
            if (sym >= 0) {
                ptr_type = sym_dtype[sym];
            }
        }

        gen_unary();  // Get pointer address into HL

        // Generate appropriate dereference based on pointer type
        if (ptr_type == TYPE_PTR_INT) {
            // Load 16-bit value: HL = *(int*)HL
            emit_ex_de_hl();      // DE = address
            emit(0x1A);           // LD A,(DE) - low byte
            emit_ld_l_a();        // L = low byte
            emit_inc_de();        // DE++
            emit(0x1A);           // LD A,(DE) - high byte
            emit_ld_h_a();        // H = high byte
        } else {
            // Load 8-bit value and zero-extend (char* or unknown)
            emit_ld_a_hl();
            emit_ld_l_a();
            emit_ld_h_n(0);
        }
...
void gen_bitand() {
    ...
    while (tok == TK_AMP) {
        ...
        emit_pop_de();
        // DE & HL -> HL
        emit_ld_a_e();       // A = E (low byte of left)
        emit(0xA5);          // AND L
        emit_ld_l_a();       // L = result low
        emit_ld_a_d();       // A = D (high byte of left)
        emit(0xA4);          // AND H
        emit_ld_h_a();       // H = result high
    }
}
...
void gen_expr() {
    ...
    // Check for pointer dereference assignment: *p = value
    if (tok == TK_STAR) {
        next();  // consume '*'
        if (tok == TK_IDENT) {
            char name[64];
            ...
            if (tok == TK_EQ) {
                ...
                if (sym_type[sym] == 1 || sym_type[sym] == 2) {
                    ...
                    // Global
                    emit_ld_hl_mem(sym_value[sym]);
                }
                emit_push_hl();  // Save pointer address
                gen_expr();  // value in HL
                emit_pop_de();  // DE = address to write to
                // Store based on pointer type
                if (ptr_type == TYPE_PTR_INT) {
                    // Store 16-bit value
                    emit_ld_a_l();
                    emit(0x12);  // LD (DE),A - low byte
                    emit_inc_de();
                    emit_ld_a_h();
                    emit(0x12);  // LD (DE),A - high byte
                } else {
                    // Store 8-bit value (char*)
                    emit_ld_a_l();
                    emit(0x12);  // LD (DE),A
                }
                return;
            }
            // Not an assignment - this is a dereference read: *p in expression
            // Generate code to load the pointer value, then dereference it
            if (sym_type[sym] == 1 || sym_type[sym] == 2) {
                emit_ld_l_ix_d(sym_value[sym]);
                emit_ld_h_ix_d(sym_value[sym] + 1);
            } else {
            ...
            // Value is now in HL. Just return - don't try to parse more.
            // This handles simple cases like print_num(*p) or x = *p
            return;
        }
        // Other cases after * (like *expr) - not supported in self-hosted
        error("Unexpected token");
    }
...
void gen_stmt() {
    ...
    if (tok == TK_INT || tok == TK_CHAR_KW || tok == TK_FLOAT) {
        ...
        // Check for pointer declaration: int *name or char *name
        if (tok == TK_STAR) {
            if (base_type == TYPE_INT) {
                var_type = TYPE_PTR_INT;
            } else if (base_type == TYPE_CHAR) {
                var_type = TYPE_PTR_CHAR;
            } else {
                var_type = TYPE_PTR;  // float * or other
            }
        ...
        // Save name and advance
        {
            ...
            if (tok == TK_LBRACKET) {
                ...
                // Arrays stored as pointers with element type info
                if (base_type == TYPE_INT) {
                    var_type = TYPE_PTR_INT;
                } else if (base_type == TYPE_CHAR) {
                    var_type = TYPE_PTR_CHAR;
...
void gen_toplevel() {
    ...
    // Check for pointer type: int *name or char *name
    if (tok == TK_STAR) {
        if (base_type == TYPE_INT) {
            var_type = TYPE_PTR_INT;
        } else if (base_type == TYPE_CHAR) {
    ...
    if (tok == TK_LBRACKET) {
        ...
        expect(TK_RBRACKET);
        var_size = elem_size * arr_count;
        // Arrays decay to pointers with element type info
        if (base_type == TYPE_INT) {
            var_type = TYPE_PTR_INT;
        } else if (base_type == TYPE_CHAR) {
            var_type = TYPE_PTR_CHAR;
